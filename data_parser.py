...
BAT_TRACKING_URL = (
    ...
    "?csv=true&year={year}&pitchType={code}"
)
...
def _pct(numerator: int, denominator: int, default: int) -> int:
    """
    Integer percentage, clamped to [1, 99].
    Returns default if denominator is 0.
    """
    if denominator == 0:
        return default
...
def _get_squared_up(mlbam_id: int, group: str, season: int) -> float | None:
    ...
    Returns a float in [0, 1], or None if the player has no data for
    that group.
    """
...
def get_pitcher_stats(name: str, season: int = SEASON) -> dict:
    ...
    if df.empty:
        raise ValueError(
            f"No Statcast data found for pitcher '{name}' in {season}."
        )
    ...
    if total == 0:
        raise ValueError(
            f"No recognisable pitch types for '{name}' in {season}.")
    ...
    for group, prefix in [
        ...
        if len(g) >= MIN_SAMPLES:
            ...
            print(
                f"  [warn] {name}: only {len(g)} {group} pitches "
                "— using default zone%"
            )
            zone = DEFAULTS["ZONE"]
...
def get_batter_stats(name: str, season: int = SEASON) -> dict:
    ...
    Sources:
      - Swing% per pitch type (Statcast pitch-by-pitch)
        → B_*_SWING, B_*_TAKE
      - Whiff% per pitch type (Statcast pitch-by-pitch)
        → B_*_WHIFF, B_*_CONTACT
      - Contact quality (Bat Tracking leaderboard)
        → B_*_FOUL, B_*_OUT, B_*_HIT
    ...
    if df.empty:
        raise ValueError(
            f"No Statcast data found for batter '{name}' in {season}."
        )
    ...
    print(
        f"[batter]  Fetching Bat Tracking leaderboard for {name} "
        f"({season})...")
    stats = {}
    ...
    for group, prefix in [
        ...
        if len(g) < MIN_SAMPLES:
            ...
            swing, take = _fix_pair(
                _pct(len(swings), len(g), DEFAULTS["SWING"]))

            # ── Whiff rate on swings (Statcast) ─────────────────────────────
            whiffs = swings[swings["description"].isin(WHIFF_DESCS)]
            whiff, contact = _fix_pair(
                _pct(len(whiffs), len(swings), DEFAULTS["WHIFF"]))
            ...
            if sq_up is not None:
                ...
                print(
                    f"  [warn] {name}: no bat tracking data for {group} "
                    "— using defaults"
                )
                foul = DEFAULTS["FOUL"]
                out = DEFAULTS["OUT"]
...
def get_matchup(
    ...
    """
    Returns combined dict with all P_* and B_* keys for a pitcher vs batter
    matchup.
...
def generate_pcsp_defines(matchup: dict) -> str:
    """
    Returns a PCSP# #define block string ready to prepend to
    baseball_template.pcsp. Paste this at the top of the template
    (replacing the existing #define block).
    """
    sections = [
        ...
        ("// On-contact outcome — foul / fielded-out / base-hit "
         "(each triple sums to 100)",
         ["B_FAST_FOUL",  "B_FAST_OUT",  "B_FAST_HIT",
          "B_BREAK_FOUL", "B_BREAK_OUT", "B_BREAK_HIT",
          "B_OFF_FOUL",   "B_OFF_OUT",   "B_OFF_HIT"]),
    ]
...
# ── CLI / smoke test ────────────────────────────────────────────────────────
# Change the pitcher and batter names here to fetch different matchups,
# or run this file directly to see the output for Cole vs Judge.
